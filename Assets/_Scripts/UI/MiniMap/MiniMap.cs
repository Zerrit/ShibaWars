...
public class MiniMap : MonoBehaviour
{
    ...
    private void ActualizeIconsCount()
    {
        ...
        else if (rightIconCount > newIconCount)
        {
    ...
    private void AddRightIcons(int count)
    {
        for (int i = 0; i < count; i++)
        {
            _rightIcons.Add(_rightIconsPool.GetFreeElement());
        }
    }
    private void RemoveRightIcons(int count)
    {
        for (int i = 0; i < count; i++)
        {
            _rightIcons[rightIconCount - 1].DisableUnnecessity();
            _rightIcons.RemoveAt(rightIconCount - 1);
        }
    }
