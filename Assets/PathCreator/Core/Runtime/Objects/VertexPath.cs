...
namespace PathCreation {
    ...
    public class VertexPath {
        ...
        {
            TimeOnPathData data = CalculateCustomPointOnPathData(worldPoint);
            return Vector3.Lerp(GetPoint(data.previousIndex), GetPoint(data.nextIndex), data.percentBetweenIndices);
        }
        ...
        TimeOnPathData CalculateCustomPointOnPathData(Vector3 worldPoint)
        {
            ...
            Vector3 customPoint = Vector3.zero;
            int customSegmentIndexA = 0;
            int customSegmentIndexB = 0;
            ...
            for (int i = 0; i < localPoints.Length; i++)
            {
                ...
                Vector3 closestPointOnSegment = MathUtility.CustomPointOnLineSegment(worldPoint, GetPoint(i), GetPoint(nextI));
                float sqrDst = Mathf.Abs(worldPoint.x - closestPointOnSegment.x);
                if (sqrDst < closestPoint)
                {
                    closestPoint = sqrDst;
                    customPoint = closestPointOnSegment;
                    customSegmentIndexA = i;
                    customSegmentIndexB = nextI;
                }

            }
            float closestSegmentLength = (GetPoint(customSegmentIndexA) - GetPoint(customSegmentIndexB)).magnitude;
            float t = (customPoint - GetPoint(customSegmentIndexA)).magnitude / closestSegmentLength;
            return new TimeOnPathData(customSegmentIndexA, customSegmentIndexB, t);
        }
